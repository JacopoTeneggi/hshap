...
def init_hexp():
    ...
    dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=50, shuffle=True, num_workers=0
    )
...
def gradexp_explain(gradexp, image):
    ...
    gradexp_shapley_values, gradexp_indexes = gradexp.shap_values(
        _input, ranked_outputs=2, nsamples=10
    )
...
exp_mapper = [
    {"name": "hexp", "init": init_hexp, "explain": hexp_explain},
    {"name": "gradexp", "init": init_gradexp, "explain": gradexp_explain},
    {"name": "deepexp", "init": init_deepexp, "explain": deepexp_explain},
    {
        "name": "gradcam",
...
for exp in exp_mapper[2:]:
    ...
    for i, image_path in enumerate(true_positives.item()["1"]):
        ...
        print(
            "%s: %d/%d runtime=%.4fs"
            % (exp_name, i + 1, len(true_positives.item()["1"]), runtime)
        )
        np.save(
            "true_positive_explanations/%s/%s" % (exp_name, image_name), explanation
        )
