...
L = len(examples)
for exp_name in exp_mapper[1:3]:
    ...
    for i, image_path in enumerate(examples):
        ...
        tmp = hshap.utils.compute_perturbed_logits(model, ref, image, explanation, perturbation_sizes, normalization="original", batch_size=20)
        exp_logits[i, :] = tmp
    ...
    np.save(os.path.join("LOR", exp_name, "results"), exp_logits.cpu().numpy())
