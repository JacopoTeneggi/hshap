...
def saveImage(im, crosses, number):
    ...
    for i in range(HEIGHT):
        ...
        for j in range(WIDTH):
            ...
            # SHAPE IS A CROSS
                ...
                truth.append((top_left, bottom_right))
    ...
    filename = os.path.join(
        HOME, "/export/gaon1/data/jteneggi/data/synthetic/LOR/images/%d/ex%d_%d.png" % (crosses, crosses, number)
    )
    cv2.imwrite(filename, image)
    np.save("/export/gaon1/data/jteneggi/data/synthetic/LOR/ground_truth/%d/ex%d_%d" % (crosses, crosses, number), truth)

def main():

    # NUMBER OF CROSSES IN LOR DATASET
    crosses_classes = np.arange(1, 10)
    ...
    for crosses in crosses_classes:
        ...
        print("Generated %d images, n=%d" % (i+1, crosses))
